import ChannelApeClient from 'channelape-sdk/dist/ChannelApeClient';
...
export default abstract class ChannelApeActionsController {
    ...
    return this.channelApeClient.actions().updateHealthCheck(actionId)
      ...
      .catch((err) => {
        ...
        try {
          ...
          this.logger.warn('Headers already sent when attempting to return a 400');
        }
      });
  }
  ...
  protected getOrderRetrievalStartDate(now: moment.Moment, channelApeOpenOrdersStartDateInveralDays: string): Date {
    ...
    const absoluteStartDate = moment(new Date(Secrets.env.CHANNEL_APE_OPEN_ORDERS_START_DATE));
